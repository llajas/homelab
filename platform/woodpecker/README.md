...
![Version: 3.6.4](https://img.shields.io/badge/Version-3.6.4-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 3.15.0](https://img.shields.io/badge/AppVersion-3.15.0-informational?style=flat-square)
...
If you want to install only the server or agent you can use the [`woodpecker/server`](./charts/server/README.md) or [`woodpecker/agent`](./charts/agent/README.md) charts respectively.
...
| agent.env.WOODPECKER_BACKEND_K8S_NAMESPACE_PER_ORGANIZATION | bool | `false` |  |
...
| agent.env.WOODPECKER_MAX_WORKFLOWS | int | `10` |  |
...
| server.env | object | `{"WOODPECKER_ADMIN":"gitea_admin","WOODPECKER_GITEA":true,"WOODPECKER_GITEA_URL":"https://git.lajas.tech","WOODPECKER_HOST":"https://ci.lajas.tech","WOODPECKER_OPEN":true,"WOODPECKER_PLUGINS_PRIVILEGED":"woodpeckerci/plugin-docker-buildx:latest","WOODPECKER_WEBHOOK_HOST":"http://woodpecker-server.woodpecker"}` | Add environment variables for the server component |
...
| server.ingress.annotations | object | `{"cert-manager.io/cluster-issuer":"letsencrypt-prod"}` | Add annotations to the ingress |
| server.ingress.enabled | bool | `false` | Enable the ingress for the server component |
| server.ingress.grpc.annotations | object | `{}` | Add annotations to the GRPC ingress |
| server.ingress.grpc.enabled | bool | `false` | Enable the GRPC ingress for the server component |
| server.ingress.grpc.hosts | list | `[{"host":"ci.lajas.tech","paths":[{"path":"/"}]}]` | Defines which ingress controller will implement the GRPC resource ingressClassName: 'nginx'  # Migrated to Gateway API HTTPRoute |
| server.ingress.grpc.tls | list | `[]` |  |
| server.ingress.hosts | list | `[{"host":"ci.lajas.tech","paths":[{"backend":{"serviceName":"chart-example.local","servicePort":80},"path":"/"}]}]` | Defines which ingress controller will implement the resource ingressClassName: 'nginx'  # Migrated to Gateway API HTTPRoute |
| server.ingress.tls[0].hosts[0] | string | `"ci.lajas.tech"` |  |
| server.ingress.tls[0].secretName | string | `"woodpecker-tls-certificate"` |  |
